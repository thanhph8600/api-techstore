...
export class CartSelectService {
  constructor(
    @InjectModel(CartSelect.name)
    private readonly cartSelectModal: Model<CartSelect>,
  ) {}
  ...
  async update(id: string, updateCartSlecteDto: any): Promise<CartSelect> {
    ...
    if (!CartSelect) {
      ...
      const checkProductPriceId = CartSelect.listProductSelect.find(
  ...
  async selectAll(id: string, updateCartSlecteDto: any): Promise<CartSelect> {
    ...
    if (updateCartSlecteDto.type === 'true') {
      ...
      const CartSelect = await this.cartSelectModal
        .findOneAndUpdate({ customerId: customerId }, { listProductSelect: [] })
        .exec();
      return CartSelect;
    }
  }
