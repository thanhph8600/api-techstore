...
export class AddressService {
  ...
  async create(createAddressDto: CreateAddressDto): Promise<HttpException> {
    try {
      ...
      return new HttpException("Địa chỉ đã được thêm thành công", HttpStatus.CREATED)
    } catch (error) {
  ...
  async updateAddress(addressId: string, updateAddressDto: UpdateAddressDto): Promise<{message: string}> {
  ...
  async delete(addressId: string): Promise<{ message: string }> {
    const deletedAddress = await this.addressModel.findByIdAndDelete(addressId).exec();
    if (!deletedAddress) {
  ...
  async setDefaultAddress(addressId: string, customerId: string): Promise<string> {
    const session = await this.addressModel.db.startSession();
    session.startTransaction()

    try {

      await this.addressModel.updateMany({ customerId, isDefault: true}, {isDefault: false}, {session})

      const updateDefaultAddress = await this.addressModel.findByIdAndUpdate(addressId, {isDefault: true}, {new: true, session}).exec()
  ...
  async findAllByCustomerId(customerId: string): Promise<Address[]> {
    // return this.addressModel.find({ customerId }).exec();
    return this.addressModel.find({ customerId }).sort({ isDefault: -1 }).exec();
  }
}
