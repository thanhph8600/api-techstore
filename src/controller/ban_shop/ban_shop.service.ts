...
export class BanShopService {
  constructor(
    @InjectModel('BanShop') private readonly BanShopModel: Model<BanShop>,
  ) {}
  ...
  async updateByIdShop(
    id_shop: string,
    updateBanShopDto: UpdateBanShopDto,
  ): Promise<BanShop | null> {
    try {
  ...
  async checkIfShopIsBanned(
    id_shop: string,
  ): Promise<{ isBanned: boolean; remainingBanTime?: number }> {
